...
# File path
file_path = "./sharc/campaigns/Guarulhos_database/Base SP 3.5 GHz Rev6 UNB.xlsx"

# Load table
db_df = pd.read_excel(file_path)
db_df.columns = db_df.columns.str.replace(' ', '_', regex=False)
...
db_df['NomeEntidade_norm'] = db_df['NomeEntidade_norm'].str.replace(',', '', regex=False) \
...
for nome, grupo in db_df.groupby("NomeEntidade_norm"):
    ...
    grupo['element_max_g'] = 5.2
    grupo.to_csv(f"./sharc/campaigns/Guarulhos_database/Database_Anatel_{nome}_elg5.2dbi.csv", sep="\t", index=False)
    grupo['Altura'] = 18.0
    grupo.to_csv(f"./sharc/campaigns/Guarulhos_database/Database_Anatel_{nome}_elg5.2dbi_h18.csv", sep="\t", index=False)
